...
public class Exercise1 {
    public static void main(String[] args) {
        ArtistHumanImpl serega = new ArtistHumanImpl("Серега", ArtistClass.ЧЕЛОВЕК);

        ArtistImpl leva = new ArtistImpl("Лёва", ArtistClass.ЛЕВ);
        ArtistImpl kesha = new ArtistImpl("Кеша", ArtistClass.ПОПУГАЙ);
        ArtistHumanImpl oleg = new ArtistHumanImpl("Олег", ArtistClass.ЧЕЛОВЕК, new CustomList(new ArtistImpl[]{leva, kesha}));
        ...
        ArtistHumanImpl leha = new ArtistHumanImpl("Леха", ArtistClass.ЧЕЛОВЕК, new CustomList(new ArtistImpl[]{potap, igor, amur}));
